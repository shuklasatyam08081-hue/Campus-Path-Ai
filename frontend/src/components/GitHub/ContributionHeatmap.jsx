...
function Skeleton() {
  return (
    <div className="w-full p-5 bg-background rounded-xl border border-border animate-pulse">
...
export default function ContributionHeatmap({ username = GITHUB_USERNAME }) {
  ...
  if (error || !data || !data.weeks) return (
    <div className="p-6 bg-background rounded-xl border border-destructive/40 text-destructive text-sm">
  ...
  return (
    <motion.div
      ...
      className="w-max min-w-full p-5 bg-background rounded-xl border border-border shadow-2xl relative select-none"
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <select
          ...
          className="bg-transparent border border-border text-foreground text-[11px] font-semibold px-2 py-1 rounded-[4px] outline-none cursor-pointer hover:bg-muted focus:ring-1 focus:ring-primary appearance-none pr-6 relative"
            ...
            {weeks.map((week, wi) => (
              <div key={wi} className="flex flex-col gap-[3px]">
                {week.contributionDays.map(day => {
                  ...
                  const bg = count > 0 ? day.color : 'var(--color-muted)';
      ...
      <AnimatePresence>
        {tooltip && (
          <motion.div
            ...
            className="bg-popover border border-[rgba(16,185,129,0.3)] rounded-md px-3 py-2 shadow-2xl whitespace-nowrap"
          >
